...
class Swagger2Importer implements SwaggerImporter {
    ...
    public RestService[] importSwagger(String url) {
        ...
        for (String path : array) {
            rtList = listToResourceTree(rtList, path, splitString(path))
        }
        System.out.println("flatTreeList enter: " + rtList.size())
        rtList = flatTreeList(rtList)
        System.out.println("flatTreeList exit: " + rtList.size())
        for (ResourceTree rt : rtList) {
            rt.print()
        }
    ...
    private List<ResourceTree> listToResourceTree(List<ResourceTree> rtList, String path, List<String> splitPath) {
        //Look for root node
        System.out.println("Adding path: " + path)
        String s = splitPath.get(0)
        ResourceTree rtPath = null
        for (ResourceTree rt : rtList) {
            if (s.equals(rt.path)) {
                //Root node found
                rtPath = rt
        ...
        if (!rtPath) {
            //Subtree root node not found. Create a new sibling
            rtPath = new ResourceTree()
            if (!rtList) {
                rtList = new ArrayList<ResourceTree>()
            }
            rtList.add(rtPath)
        }
        //Start subtree with root node
        rtPath = listToResourceTree(rtPath, path, splitPath)
    ...
    private ResourceTree listToResourceTree(ResourceTree rt, String path, List<String> splitPath) {
        for (String s : splitPath.clone()) {
            splitPath.remove(0)
            if (s.equals(rt.path)) {
                //Found node with name equals to split path
                if (splitPath.isEmpty()) {
                    //Path empty. End of tree
                    rt.fullPath = path
                    System.out.println("End of path: " + rt.fullPath)
                    return rt
                }

                //continue path with childs
                if (!rt.childs) {
                    rt.childs = new ArrayList<ResourceTree>()
                }
                rt.childs = listToResourceTree(rt.childs, path, splitPath)
                return rt
            } else {
                //Path not found. Create new one
                rt.path = s
    ...
    private List<ResourceTree> flatTreeList (List<ResourceTree> treeList) {
        for (ResourceTree tree : treeList) {
            tree = flatTree(tree)
        }
    }

    private ResourceTree flatTree (ResourceTree tree) {
        System.out.println("flatTree " + tree.path)
        if ((tree.childs == null) || (tree.childs.isEmpty())) {
            return tree
        } else {
            for (ResourceTree child : tree.childs) {
                System.out.println("flatTree child " + child.path)
                if (!flat(child)) {
                    child = flatTree(child)
                }
                if (flat(child)) {
                    System.out.println("flatTree child fullPath " + child.fullPath)
                    tree.fullPath = child.fullPath
                    tree.path = tree.path + "/" + child.path
                    tree.childs = null
                }
            }
            return tree
    ...
    private boolean flat (ResourceTree tree) {
        if ((tree.childs == null) || (tree.childs.isEmpty())) {
            return true
        } else {
            return false
